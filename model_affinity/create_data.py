...
from utils_data import DTADataset, sparse_mx_to_torch_sparse_tensor, minMaxNormalize, denseAffinityRefine
...
def get_clique_molecule_graph(ligands):
    clique_graph = OrderedDict()

    for d in ligands.keys():
        lg = Chem.MolToSmiles(Chem.MolFromSmiles(ligands[d]), isomericSmiles=True)
        clique_graph[d] = clique_to_graph(lg)
...
def get_target_molecule_graph(proteins, dataset):
    msa_path = '/home/july/PycharmProjects/23AIBox-CSCo-DTA-main/data/data/' + dataset + '/aln'
    contac_path = '/home/july/PycharmProjects/23AIBox-CSCo-DTA-main/data/data/' + dataset + '/pconsc4'
